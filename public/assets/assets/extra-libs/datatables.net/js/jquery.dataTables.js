...
(function( $, window, document, undefined ) {
	...
	var DataTable = function ( options )
	{
		 ...
		 *      oTable.fnFilter( 'sectionone string' );
		 *    } );
		 */
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the sectionone in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 * level down. On each loop we sectionone for undefined, and if found immediately
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *  }
			 * Basically we have a sectionone for each sorting column, if the data in that column is equal,
			 * sectionone the next column. If all columns match, then we use a numeric sort on the row
	 ...
	 *      selector for the table to sectionone. Note that if more than more than one
